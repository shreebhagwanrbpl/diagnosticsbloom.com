...
export default function Footer() {
  ...
  useEffect(() => {
    const loadDistrict = async () => {
      ...
      try {
        const snap = await getDoc(
          doc(db, "websites", "diagnosticsbloomcom", "districts", district)
        );
  ...
  if (loading) {
    return (
      <footer className="border-t border-[#CFE1CF] bg-[#EAF4E8]">
        <div className="container-custom py-16">
          <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
            {[...Array(4)].map((_, i) => (
              <div key={i}>
                <div className="mb-6 h-8 w-40 animate-pulse rounded bg-[#D8EAD5]" />
                {[...Array(5)].map((_, j) => (
                  <div
                    key={j}
                    className="mb-4 h-5 animate-pulse rounded bg-[#E1EFDF]"
                  />
                ))}
              </div>
            ))}
          </div>
          <div className="mt-12 border-t border-[#CFE1CF] pt-6">
            <div className="h-5 w-72 animate-pulse rounded bg-[#D8EAD5]" />
          </div>
        </div>
      </footer>
    );
  ...
  return (
    <footer className="border-t border-[#CFE1CF] bg-gradient-to-b from-white via-[#F7FBF6] to-[#EAF4E8]">
      <div className="container-custom py-14 sm:py-16">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex flex-col justify-between">
            ...
            {/* Social Media Links */}
            <div className="mt-6 pt-4 border-t border-[#CFE1CF]/60">
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#193522] mb-3">
                Follow Us
              </h4>
              <div className="flex items-center gap-3">
                <a
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-2xl border border-[#CFE1CF] bg-white text-[#1877F2] shadow-sm transition-all duration-300 hover:scale-110 hover:bg-[#1877F2] hover:text-white hover:shadow-md"
                >
                  <FaFacebook size={20} />
                </a>
                ...
                <a
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-2xl border border-[#CFE1CF] bg-white text-[#2F6B3C] shadow-sm transition-all duration-300 hover:scale-110 hover:bg-gradient-to-tr hover:from-[#2F6B3C] hover:via-[#3F7F4D] hover:to-[#24572F] hover:text-white hover:shadow-md"
                >
                  <FaInstagram size={20} />
                </a>
              </div>
          ...
          {/* Quick Links */}
          <div>
            <h3 className="mb-5 text-lg font-bold text-[#193522]">
              Quick Links
            </h3>
            <div className="flex flex-col gap-3 text-sm font-medium">
              {[
                ...
                <Link
                  ...
                  className="text-[#657566] transition-all duration-300 hover:translate-x-1.5 hover:text-[#2F6B3C] flex items-center gap-1.5"
                >
                  <ArrowRight size={14} className="text-[#2F6B3C] opacity-60" />
                  <span>{item.name}</span>
                </Link>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-2.5 text-sm font-medium">
              {displayCategories.map((cat, idx) => (
                <Link
                  ...
                  className="text-[#657566] transition-all duration-300 hover:translate-x-1.5 hover:text-[#2F6B3C] flex items-center gap-1.5"
                >
                  <span className="h-1.5 w-1.5 rounded-full bg-[#2F6B3C]" />
                  <span className="truncate">{cat}</span>
                </Link>
              ))}
              <Link
                href={makeLink("/items")}
                className="mt-2 text-xs font-bold text-[#2F6B3C] hover:underline"
              >
                View Full Catalog →
              </Link>
            </div>
          ...
          <div>
            ...
            <div className="space-y-4 text-[#657566]">
              {dynamicAddress && (
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#E1EFDF]">
                    <MapPin size={18} className="text-[#2F6B3C]" />
                  </div>
              ...
              {phones.length > 0 && (
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#E1EFDF]">
                    <Phone size={18} className="text-[#2F6B3C]" />
                  </div>
              ...
              {emails.length > 0 && (
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#E1EFDF]">
                    <Mail size={18} className="text-[#2F6B3C]" />
                  </div>
                  <div className="flex flex-col gap-1 text-sm font-semibold">
                    {emails.map((em, idx) => (
                      <a
                        key={idx}
                        href={`mailto:${em}`}
                        className="hover:text-[#2F6B3C] transition-colors break-all"
                      >
                        {em}
                      </a>
                    ))}
              ...
              {!dynamicAddress && phones.length === 0 && emails.length === 0 && (
                <p className="text-xs text-[#193522]">
        ...
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-[#CFE1CF] pt-8 text-sm text-[#6B5A4A] md:flex-row">
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#657566] hover:text-[#1877F2] transition-colors"
              aria-label="Facebook"
            >
              <FaFacebook size={18} />
            </a>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#657566] hover:text-[#2F6B3C] transition-colors"
              aria-label="Instagram"
            >
              <FaInstagram size={18} />
            </a>
            <span className="text-xs text-[#193522]">
